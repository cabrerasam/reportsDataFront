...
const ViewWeekly = () => {
  ...
  return (
    <main className='flex justify-center items-center flex-col pb-8'>
      <h1 className='text-4xl text-white my-8'>Semanal</h1>
            ...
            .map((week, index) => (
              <li key={index} className='text-white flex items-center place-self-center'>
                <div className='flex flex-col w-[420px] gap-1 bg-custom-transparent px-4 py-4'>
                  <p><span className='text-cyan-500 font-bold'>Nro de informe: </span>{week.num_weekly}</p>
                  <p><span className='text-cyan-500 font-bold'>Fecha del informe: </span>{dateConvert(week.date_weekly)}</p>
                  <p><span className='text-cyan-500 font-bold'>Temas: </span></p>
                  <ul>
                    {
                      issues
                        .filter(issue => issue.id_report === week.id_weekly)
                        .map((issue, index) => (
                          <li key={index}><span className='text-cyan-500 mr-2'>{index + 1}: </span>{issue.issue_report}</li>
                        ))
                    }
                  </ul>
                  <Link to={`/weeklyList/${week.id_weekly}`} className='text-center bg-cyan-500 w-28 px-4 mt-2'>Abrir</Link>
                </div>
              </li>
            ))
        }
