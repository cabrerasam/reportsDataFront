...
const Diary = () => {
  ...
  useEffect(() => {
    if (individualDiary) {
      const timer = setTimeout(() => setIndividualDiary(''), 2000)
  ...
  useEffect(() => {
    if (collectiveDiary) {
      const timer = setTimeout(() => setCollectiveDiary(''), 2000)
      return () => clearTimeout(timer)
    }
  }, [collectiveDiary])
  ...
  const submitForm = async (e) => {
    ...
    formData.append('typeDiary', e.target.type.value)
    formData.append('priorityDiary', e.target.priority.value)
    formData.append('confidentialityDiary', e.target.confidentiality.value)
    formData.append('numDiary', e.target.num.value)
    formData.append('dateDiary', e.target.date.value)
    formData.append('issueDiary', e.target.issues.value)
    formData.append('file', e.target.file.files[0])
    formData.append('idUser', idUser)

    await fetch(`${URL}/diaries`, {
      method: 'POST',
      body: formData
    })
      .then(res => res.json())
  ...
  const submiteIndividual = (e) => {
    ...
    fetch(`${URL}/intermediate/individualDiaries`, {
      ...
      .then(res => res.json())
      .then(data => {
        setIndividualDiary(data[0].insert_individuals_diaries.message)
      })
      .catch(error => error)
  }
  ...
  const submiteCollective = (e) => {
    ...
    fetch(`${URL}/intermediate/collectiveDiaries`, {
      ...
      .then(res => res.json())
      .then(data => {
        setCollectiveDiary(data[0].insert_collectives_diaries.message)
      })
      .catch(error => error)
  }
    ...
    <main className='ml-auto mr-auto w-2/3 h-full grid pt-[60px] place-items-center'>
      ...
      <section className='flex justify-center items-center w-[600px] h-[430px] relative overflow-hidden'>
        <form className='absolute flex flex-col items-center justify-center w-[600px] h-[430px] one' onSubmit={submitForm} encType='multipart/form-data' style={{ left: attribute === 'one' ? '0' : '-600px' }}>
          <h1 className='text-2xl text-center mb-3 text-white'>Crear informe diario</h1>
        ...
        <form className='absolute flex flex-col items-center justify-center w-[600px] h-[430px] two' onSubmit={submiteIndividual} style={{ left: attribute === 'two' ? '0' : '-600px' }}>
        ...
        <form className='absolute flex flex-col items-center justify-center w-[600px] h-[430px] three' onSubmit={submiteCollective} style={{ left: attribute === 'three' ? '0' : '-600px' }}>
          ...
          <fieldset className='flex flex-col border border-cyan-900 p-3 gap-4'>
            ...
            <div className='flex justify-center gap-8'>
              <ButtonLight
                ...
                typeButton='submit'
              />
              <a className='w-24 h-10 bg-red-500 hover:bg-red-600 text-white rounded-2xl flex justify-center items-center' href='/diaryList'>Finalizar</a>
            </div>
          </fieldset>
        </form>
      </section>
      <p className={`${diary.id_diary ? 'text-green-500' : 'text-red-500'} text-center pt-2`} style={attribute !== 'one' ? { display: 'none' } : { display: 'block' }}>
        {
          diary.id_diary ? 'El informe ha sido creado exitosamente, diríjase al PASO 2' : 'El informe aún no ha sido creado'
        }
      </p>
      <p className={`${individualDiary !== '' ? 'text-green-500' : 'text-red-500'} text-center pt-2`} style={attribute !== 'two' ? { display: 'none' } : { display: 'block' }}>
        {
          individualDiary !== '' ? 'Puede seguir agregando actores o continuar con el PASO 3' : 'Agregue actores al informe'
        }
      </p>
      <p className={`${collectiveDiary !== '' ? 'text-green-500' : 'text-red-500'} text-center pt-2`} style={attribute !== 'three' ? { display: 'none' } : { display: 'block' }}>
        {
          collectiveDiary !== '' ? 'Puede seguir agregando colectivos o finalizar' : 'Agregue colectivos al informe'
        }
      </p>
    </main>
  )
