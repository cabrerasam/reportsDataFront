...
const SingleSunday = () => {
  ...
  useEffect(() => {
    fetch(`${URL}/sundays`)
      .then(res => res.json())
      .then(data => setSunday(data.filter(sunday => sunday.id_sunday === id)))
      .catch(error => error)
  }, [id])
  ...
  useEffect(() => {
    fetch(`${URL}/intermediate/individualSunday`)
      .then(res => res.json())
      .then(data => setIndividualSunday(data.filter(indSunday => indSunday.id_sunday === id)))
      .catch(error => error)
  }, [id])
  ...
  useEffect(() => {
    fetch(`${URL}/individuals`)
      .then(res => res.json())
      .then(data => setIndividuals(data.filter(ind => ind.id_individual === individualSunday[0].id_individual)))
      .catch(error => error)
  }, [individualSunday])
  ...
  useEffect(() => {
    fetch(`${URL}/intermediate/collectiveSunday`)
      .then(res => res.json())
      .then(data => setCollectiveSunday(data.filter(colSunday => colSunday.id_sunday === id)))
      .catch(error => error)
  }, [id])
  ...
  useEffect(() => {
    fetch(`${URL}/collectives`)
      .then(res => res.json())
      .then(data => setCollectives(data.filter(col => col.id_collective === collectiveSunday[0].id_collective)))
      .catch(error => error)
  }, [collectiveSunday])
          ...
          <h3 className='text-center font-bold text-cyan-500 text-xl'>Informe diario</h3>
          {
            sunday.map((day, index) => (
              <div key={index} className='flex flex-col gap-2 text-white'>
                <p><span className='text-cyan-500'>Tipo de informe: </span>{day.type_sunday}</p>
                <p><span className='text-cyan-500'>Prioridad: </span>{day.priority_sunday}</p>
                <p><span className='text-cyan-500'>Confidencialidad: </span>{day.confidentiality_sunday}</p>
                <p><span className='text-cyan-500'>Nro de informe: </span>{day.num_sunday}</p>
                <p><span className='text-cyan-500'>Fecha del informe: </span>{dateConvert(day.date_sunday)}</p>
                <p><span className='text-cyan-500'>Temas del informe: </span>{day.issue_sunday}</p>
                <a href={`${URL}${day.link_sunday}`} className='bg-custom-red px-4 py-1 text-center mt-4' target='_blanck'>Abrir informe</a>
              </div>
            ))
          }
        </div>
