...
const Sunday = () => {
  ...
  const [individualSunday, setIndividualSunday] = useState('')
  const [collectiveSunday, setCollectiveSunday] = useState('')
  ...
  const submitForm = (e) => {
    ...
    formData.append('typeSunday', e.target.type.value)
    formData.append('prioritySunday', e.target.priority.value)
    formData.append('confidentialitySunday', e.target.confidentiality.value)
    formData.append('numSunday', e.target.num.value)
    formData.append('dateSunday', e.target.date.value)
    formData.append('issueSunday', e.target.issues.value)
  ...
  const submiteIndividual = (e) => {
    ...
    fetch(`${URL}/intermediate/individualSunday`, {
      ...
      .then(res => res.json())
      .then(data => {
        setIndividualSunday(data[0].insert_individuals_sundays.message)
      })
      .catch(error => error)
  }
  ...
  const submiteCollective = (e) => {
    ...
    fetch(`${URL}/intermediate/collectiveSunday`, {
      ...
      .then(res => res.json())
      .then(data => {
        setCollectiveSunday(data[0].insert_collectives_sundays.message)
      })
      .catch(error => error)
  }
    ...
    <main className='ml-auto mr-auto w-2/3 h-full grid pt-[60px] place-items-center'>
      ...
      <section className='flex justify-center items-center w-[600px] h-[430px] relative overflow-hidden'>
        <form className='absolute flex flex-col items-center justify-center w-[600px] h-[430px] one' onSubmit={submitForm} encType='multipart/form-data' style={{ left: attribute === 'one' ? '0' : '-600px' }}>
          <h1 className='text-2xl text-center mb-3 text-white'>Crear informe dominical</h1>
        ...
        <form onSubmit={submiteCollective} className='absolute flex flex-col items-center justify-center w-[600px] h-[430px] three' style={{ left: attribute === 'three' ? '0' : '-600px' }}>
      ...
      <p className={`${sunday.id_sunday ? 'text-green-500' : 'text-red-500'} text-center pt-2`} style={attribute !== 'one' ? { display: 'none' } : { display: 'block' }}>
        {
          sunday.id_sunday ? 'El informe ha sido creado exitosamente, diríjase al PASO 2' : 'El informe aún no ha sido creado'
        }
      </p>
      <p className={`${individualSunday !== '' ? 'text-green-500' : 'text-red-500'} text-center pt-2`} style={attribute !== 'two' ? { display: 'none' } : { display: 'block' }}>
        {
          individualSunday !== '' ? 'Puede seguir agregando actores o continuar con el PASO 3' : 'Agregue actores al informe'
        }
      </p>
      <p className={`${collectiveSunday !== '' ? 'text-green-500' : 'text-red-500'} text-center pt-2`} style={attribute !== 'three' ? { display: 'none' } : { display: 'block' }}>
        {
          collectiveSunday !== '' ? 'Puede seguir agregando colectivos o finalizar' : 'Agregue colectivos al informe'
        }
      </p>
    </main>
  )
